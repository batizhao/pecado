...
public class AppTableController {
     ...
     * @return
     */
    @Operation(description = "查询可以查询的字段")
    @GetMapping("/table/search/{id}")
    public R<List<AppTableColumn>> handleTableSearchColumn(@Parameter(name = "ID" , required = true) @PathVariable("id") @Min(1) Long id) {
        return R.ok(appTableService.listSearchColumn(id));
    }
