...
public class DsApiTest extends BaseApiTest {
    ...
    public void givenJson_whenSaveDs_thenSuccess() throws Exception {
        Ds requestBody = new Ds()
                .setName("system")
                .setUrl("jdbc:mysql://localhost:3306/pecado-system?useUnicode=true&useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai")
                .setUsername("root")
    ...
    public void givenId_whenDeleteDs_thenSuccess() throws Exception {
        mvc.perform(delete("/ds").param("ids", "1")
                .header("Authorization", adminAccessToken))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
